...
def _toom_cook_nonnegative(x: int, y: int, parts: int, threshold: int) -> int:
    ...
    sample_points = list(range(2 * parts - 1))
    ...
    for x_value, y_value in zip(x_values, y_values):
        sampled_products.append(_toom_cook_nonnegative(x_value, y_value, parts, threshold))
...
def toom_cook_multiply(x: int, y: int, parts: int = 3, threshold: int = 2) -> int:
    if parts < 2:
...
#     chunk_digits = math.ceil(max(digit_count(left), digit_count(right)) / parts)
#     base = 10 ** chunk_digits
#     left_chunks = split_into_chunks(left, chunk_digits, parts)
#     right_chunks = split_into_chunks(right, chunk_digits, parts)

#     product = toom_cook_multiply(left, right, parts=parts)
#     print("Example: 1234 * 5678 with 3-way Toom-Cook")
#     print(f"Chunk base: {base}")
#     print(f"1234 -> coefficients {left_chunks}")
#     print(f"5678 -> coefficients {right_chunks}")
#     print(f"Got: {product}")
#     print(f"Basic multiplication check: {left * right}")


# def run_small_tests() -> None:
#     test_cases = [
#         (12, 34, 2, 408),
#         (123, 456, 3, 56088),
#         (1234, 5678, 3, 7006652),
...
#     for left, right, parts, expected in test_cases:
#         actual = toom_cook_multiply(left, right, parts=parts)
#         assert actual == expected, (
#             f"Test failed for {left} * {right} with parts={parts}: "
...
def solve():
    ...
    for i in range(0, len(input_data), 2):
        ...
        # (Defaults to parts=3, threshold=2)
