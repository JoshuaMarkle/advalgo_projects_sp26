...
public:
    ...
    //function for getting the k shortest paths
    std::vector<std::pair<std::vector<int>, int>> k_shortest_paths(int k) {
        ...
        final_res.emplace_back(construct_path_from_sidetrack_edges({}), d_source_target);
        if (k == 1 || !adj_heaps[source]) return final_res; //return early if there are no sidetrack edges out of the source
        ...
        //we either return k shortest paths, or all the paths if k is larger than the number of paths. If there are no paths left
        //to consider, the priority queue will be empty, since there are no more sidetrack edges to consider. 
        while (!pq.empty() && final_res.size() < k) {
            State curr = pq.top();
            pq.pop();
            //reconstruct path
            final_res.emplace_back(construct_path_from_sidetrack_edges(curr.sidetrack_edges), d_source_target + curr.dist);
            auto H = curr.next;
...
int main() {
    ...
    std::vector<std::pair<std::vector<int>, int>> res = eppsteins.k_shortest_paths(k);
    for (auto p : res) {
        std::cout << p.second << std::endl;
        for (int node : p.first) {
            std::cout << node << " ";
        }
        std::cout << std::endl;
    }
